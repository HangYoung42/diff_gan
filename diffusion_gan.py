...
from model_ import Discriminator
from model_ import MappingNetwork
...
# Initialize generator and discriminator and so on
generator = Generator(5,28*28)
...
mapping_network = MappingNetwork(d_latent, mapping_network_layers)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if torch.rand(()).item() < style_mixing_prob:
            ...
            # Sample $z_1$ and $z_2$
            z2 = torch.randn(batch_size, d_latent).cuda()
            z1 = torch.randn(batch_size, d_latent).cuda()
            # Get $w_1$ and $w_2$
        ...
        # Without mixing
            ...
            # Sample $z$ and $z$
            z = torch.randn(batch_size, d_latent).cuda()
            # Get $w$ and $w$
            w = mapping_network(z)
